...
python run_text_classification.py \
  ...
  --multisource_data "fs_ttrain/americas_nli_${TARGET_LANG}_fs_ttrain.json" \
...
mkdir -p "$RESULTS_DIR"
TASK_ADAPTER="${DIR}/nli"

python run_text_classification.py \
  --model_name_or_path "$BASE_MODEL" \
  --task_ft "$DIR" \
  --load_adapter "${TASK_ADAPTER}" \
  --task_name nli \
  --source_lang "$TARGET_LANG" \
  --source_test_file "${PROJECT_ROOT}/datasets/AmericasNLI/test/${TARGET_LANG}.tsv" \
  --label_file "${PROJECT_ROOT}/datasets/AmericasNLI/labels.json" \
  --input_columns "premise" "hypothesis" \
  --adaptation_method "$ADAPTATION_METHOD" \
  --source_adaptation "$LANGUAGE_ADAPTATION" \
  --lang_adapt_format "$ADAPTATION_PATTERN" \
  ...
  --eval_split "$EVAL_SPLIT" > "$RESULTS_DIR/log_${SOURCE_LANG}.txt" 2>&1
