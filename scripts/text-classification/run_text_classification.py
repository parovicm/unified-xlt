...
class DataTrainingArguments:
    ...
    task_name: Optional[str] = field(
        default=None,
        metadata={"help": "Name of the task: nli or sa."},
    )
...
def main():
    ...
    model_args, data_args, fs_args, sft_args, training_args, adapter_args = parser.parse_args_into_dataclasses()
    ...
    raw_datasets = load_few_shot_data(
        fs_args,
        training_args,
    ...
    if fs_args.adaptation_method == 'sft':
        lang_sfts = load_sfts(fs_args)
        logger.info(lang_sfts)
    elif fs_args.adaptation_method == 'adapters':
        lang_adapters = load_adapters(fs_args, model, adapter_args, data_args.task_name)
    else:
        raise ValueError(f'Adaptation method "{fs_args.adaptation_method}" is not supported.')

    if fs_args.adaptation_method == 'sft' and sft_args.task_ft is not None:
    ...
    def preprocess_function(examples):
        ...
        tokenized_examples['label'] = [
            label2id[str(label)] if label not in range(num_labels) else label
    ...
    if training_args.do_train:
        ...
        if training_args.local_rank <= 0 and fs_args.adaptation_method == 'sft':
